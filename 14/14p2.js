...
// let cave = Array(maxX - minX + 1).fill(0); // width
let cave = Array((maxY + 3) * 2 + 1).fill(0); // width
cave = cave.map((x) => {
  const arr = Array(maxY + 2 + 1).fill("."); // height
  return [...arr];
});
//center X shift
console.log(minX);
minX = 500 - Math.floor(cave.length / 2);
console.log(minX);
